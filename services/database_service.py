...
class SQLDatabase:
    def __init__(self) -> None:
        ...
        connection_string = (
            f"postgresql+psycopg2://{self._user_name}:{self._password}@{self._endpoint}:{self._port}/dlmdspwp01"
        )
        print(f"Setting up sql alchemy engine with the connection string: {connection_string}")
        self._engine = create_engine(connection_string, pool_pre_ping=True, echo=True)
